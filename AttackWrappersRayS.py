...
def RaySAttack(model, epsMax, queryLimit, cleanLoader, num_clean_images):
    
    if not os.path.exists(f'Images/Adv_images_{epsMax}'):
        os.makedirs(f'Images/Adv_images_{epsMax}')
    
    if not os.path.exists(f'Images/Clean_images_{epsMax}'):
        os.makedirs(f'Images/Clean_images_{epsMax}')
    ...
    xAdv = xAdv[0:num_clean_images]
    yClean = yClean[0:num_clean_images]

    for i in range(0, num_clean_images):
        print(i)
        start = time.time()
        ...
        if dist>epsMax:
            print(f"Attack failed on Image {i}, returning clean sample instead.")
            xAdv[i] = xClean[i]

            with open(f"data_ids/failed_attacks_{epsMax}.txt", "a") as file:
                file.write(str(i))
                file.write("\n")
            ...
            with open(f"data_ids/success_attacks_{epsMax}.txt", "a") as file:
                file.write(str(i))
                file.write("\n")

        pil_image = ToPILImage()(xAdvCurrent)
        pil_image.save(f"Images/Adv_images_{epsMax}/Adv_image_{i}.png")

        pil_image = ToPILImage()(xClean[i])
        pil_image.save(f"Images/Clean_images_{epsMax}/Clean_image_{i}.png")
